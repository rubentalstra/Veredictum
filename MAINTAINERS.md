...
| `GITHUB_TOKEN` (ephemeral, per workflow run) | the GitHub release, and the GHCR container image once the release lane lands | arrives with the CI lanes (#2789) | GitHub, minted per run; nothing is stored | not applicable. There is no credential to lose |
