...
# repository: the catalogue, the fixture declarations, and the vendored packs.
#
...
# ── The party documents ────────────────────────────────────────────────────
# The ICS and IXIT fixture declarations, plus the committed example results
# document (#48, examples/results.example.json — generated by the crate's own
# machinery via `cargo run --example make_example_results`), so the results
# half of the harness starts from a real, schema-valid record rather than from
# mutations of a declaration.
seed_party_document() {
  link_from party_document fixtures/declaration 256k 40 '*.json'
