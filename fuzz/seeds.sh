...
# ── The party documents ────────────────────────────────────────────────────
# The committed statements and IXIT declarations. There are four of them, which
# is the entire population: no party has published a results record into this
# repository yet, so the results half of the harness starts from mutations of a
# statement rather than from a real record.
seed_party_document() {
  link_from party_document party 256k 40 '*.json'
}

# ── The HDR histogram V2 decoder ───────────────────────────────────────────
# No encoded histogram is committed in this repository, so these are written:
# the V2 cookie (0x1C849303, HdrHistogram's own encoding constant) followed by
# plausible and implausible header bytes. The harness base64-encodes whatever it
# is given, so a seed is RAW BYTES.
seed_hdr_v2() {
  ...
  echo "  hdr_v2 <- written V2 encodings (5 seeds)"
}
