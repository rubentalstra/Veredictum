...
The `org.openehr.lang.bmm3.model_access` package provides an interface for the application to load BMM schemas and convert them to BMM model form, and is shown below. In this model, a _schema_ is the serial form of a _model_ or part of a model (i.e. instances of a persistence model like the `lang.bmm_persistence` model `P_XXX` classes). One or more schema files are parsed, validated and then converted to create a single `BMM_MODEL` instance.

[.text-center]
.`org.openehr.lang.bmm3.model_access` Package
image::{uml_diagrams_uri}/LANG-bmm3.model_access.svg[id=model_access_package, align="center"]
