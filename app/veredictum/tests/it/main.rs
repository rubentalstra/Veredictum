// SPDX-FileCopyrightText: Veredictum contributors
// SPDX-License-Identifier: Apache-2.0

//! Integration tests for `veredictum`, the CNF 2.0 conformance runner: the
//! committed catalogue's artifact gates and schema-drift guards, the
//! claim/coverage completeness gates, the vendored corpus packs, defect-fixture
//! rejection, the measured-performance driver, the universal-benchmark engine,
//! the published pack manifest, the public boards' committed submissions and
//! registry entries, the self-verification pack, and the wire-speaking modules
//! against the `fake_sut` harness.
//!
//! One binary per crate, split into topic modules: Cargo compiles and links
//! every top-level `tests/*.rs` as its own crate
//! (<https://doc.rust-lang.org/cargo/reference/cargo-targets.html#integration-tests>),
//! so one binary saves the link waste while nextest still runs each test in
//! its own process.

#![expect(
    clippy::disallowed_types,
    reason = "an independently authored wire input catches codec bugs a typed-then-serialized value cannot, so fixtures and wire assertions are raw JSON"
)]

mod artifact_gates;
mod bench_engine;
mod bench_manifest;
mod bench_submissions;
mod claim_completeness;
mod console_submission;
mod corpus_packs;
mod defect_rejection;
mod driver_wire;
mod env_coupled;
mod fake_sut;
mod folder_provisioning;
mod gpg_interop;
mod measured_wire;
mod perf_driver;
mod perf_seeding;
mod pipeline_seams;
mod postconditions;
mod provisioning_wire;
mod rederivation;
mod registry_submissions;
mod run_execution;
mod run_transcript;
mod schema_drift;
mod signature_asserts;
mod unparsed_bodies;
mod verification_pack;
mod version_asserts;
